...
token = '6031589759:AAG2iOKUzWDETo316ohqp9QD-g76Kn4K7S8'  # <<< Ваш токен
...
def oga2wav(filename):
    ...
    #bot.send_message(message.chat.id, new_filename)
...
def download_file(bot, file_id):
    ...
    # bot.send_message(message.chat.id, "uhepbv")
    file_info = bot.get_file(file_id)
    downloaded_file = bot.download_file(file_info.file_path)
    filename = file_id + file_info.file_path
    filename = filename.replace('/', '_')
    #bot.send_message(message.chat.id, 'Файл '+filename)
...
def transcript(message):
    bot.send_message(message.chat.id, 'это звук')
